...
lint-py:
	...
	# Requires flake8
	$(WORKON_HOME)/AQIService/bin/flake8 .
...
setup:
	virtualenv -p `which python3.7` $(WORKON_HOME)/AQIService
	$(WORKON_HOME)/AQIService/bin/pip install -U pip wheel
	$(WORKON_HOME)/AQIService/bin/pip install -Ur requirements/dev.txt
	$(WORKON_HOME)/AQIService/bin/pip freeze
	npm install
	npm update
	cp AQIService/settings/local.example.py AQIService/settings/local.py
	echo "DJANGO_SETTINGS_MODULE=AQIService.settings.local" > .env
	createdb -E UTF-8 AQIService
	$(WORKON_HOME)/AQIService/bin/python manage.py migrate
	if [ -e project.travis.yml ] ; then mv project.travis.yml .travis.yml; fi
	@echo
	@echo "The AQIService project is now setup on your machine."
	@echo "Run the following commands to activate the virtual environment and run the"
	@echo "development server:"
	@echo
	@echo "	workon AQIService"
	@echo "	npm run dev"

update:
	$(WORKON_HOME)/AQIService/bin/pip install -U -r requirements/dev.txt
	npm install
	npm update
